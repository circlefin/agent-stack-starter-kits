...
export function buildTools(ask: (q: string) => Promise<string>) {
  ...
  const getWalletBalance = tool(
    ...
    },
    {
      ...
      description: `Check USDC and token balances for a wallet address. Defaults to Base; pass chain "POLYGON" to read the Polygon balance.`,
      schema: z.object({
        address: z.string().describe('EVM wallet address (0x...).'),
        chain: chainEnum
          .optional()
          .describe('Chain to read the balance on. Defaults to BASE.'),
      }),
    },
  );
  ...
  const deployWalletTool = tool(
    ...
    },
    {
      ...
      schema: z.object({
        ...
        chain: chainEnum
          .optional()
          .describe('Chain to deploy the SCA on. Defaults to BASE.'),
      }),
    },
  );
  ...
  const payService = tool(
    async ({ url, address, dataJson, method }) => {
      ...
      // payment. Deployment is per-chain, so check the chain being paid. Catch it
      // here with an actionable message instead of the CLI's opaque "Could not
      // sign payment authorization" failure.
      try {
  ...
  const getGatewayBalance = tool(
    ...
    },
    {
      ...
      schema: z.object({
        ...
        chain: chainEnum
          .optional()
          .describe('Chain to read the Gateway balance on. Defaults to BASE.'),
      }),
    },
  );
